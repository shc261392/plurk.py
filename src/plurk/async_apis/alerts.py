...
class Alerts(BaseApi):
    ...
    async def add_as_fan(self, user_id: int):
    ...
    async def add_all_as_fan(self):
        """Accept all friendship requests as fans.
        """
        endpoint = f'{self.client.base_url}/APP/Alerts/addAllAsFan'
        resp = await self.client.http_client.post(endpoint)
        validate_resp(resp)
        return ActionResp(**resp.json())

    async def add_as_friend(self, user_id: int):
    ...
    async def add_all_as_friends(self):
    ...
    async def deny_friendship(self, user_id: int):
        """Accept user_id as friend.
        """
    ...
    async def remove_notification(self, user_id: int):
        """Accept user_id as friend.
        """
