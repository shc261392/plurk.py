...
class Realtime(BaseApi):
    ...
    async def get_channel_response(self, comet_channel_url: str, timeout: int = 120, new_offset: Optional[int] = None):
        ...
        resp = await self.client.http_client.get(endpoint, timeout=timeout)
        validate_resp(resp)
