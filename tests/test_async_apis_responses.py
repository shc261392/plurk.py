...
@pytest.mark.asyncio
async def test_get(httpx_mock: HTTPXMock, responses_get_resp_fixture: ResponsesGetResp):
    ...
    async with AsyncClient(*FAKE_APP_CREDENTIALS) as client:
        resp = await client.responses.get(FAKE_PLURK_ID)
    assert isinstance(resp, ResponsesGetResp)
    assert resp.dict_original() == resp_json


@pytest.mark.asyncio
async def test_response_add(httpx_mock: HTTPXMock, response_fixture: Response):
    ...
    async with AsyncClient(*FAKE_APP_CREDENTIALS) as client:
        response = await client.responses.response_add(FAKE_PLURK_ID, FAKE_CONTENT)
    assert isinstance(response, Response)
    assert response.dict_original() == resp_json


@pytest.mark.asyncio
async def test_response_delete(httpx_mock: HTTPXMock, action_resp_fixture: ActionResp):
    ...
    async with AsyncClient(*FAKE_APP_CREDENTIALS) as client:
        action_resp = await client.responses.response_delete(FAKE_PLURK_ID, FAKE_RESPONSE_ID)
